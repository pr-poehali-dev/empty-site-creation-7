    ...
    <div className="min-h-screen">
      ...
      <main className="max-w-3xl mx-auto px-4 py-6 sm:py-8">
        <div className="mb-5 sm:mb-6">
          <h2 className="text-xl sm:text-2xl font-semibold">Личный кабинет</h2>
          {user.role_name && (
            <Badge className="mt-2 bg-white/[0.06] text-muted-foreground border-white/[0.08]">
              {user.role_name}
            </Badge>
          )}
        </div>
        ...
        <div className="flex gap-2 mb-5 flex-wrap">
          <Button
            variant="outline"
            className="w-full sm:flex-1 h-12 rounded-xl border-white/[0.08] justify-start gap-3"
            onClick={() => navigate("/wholesaler/inventories")}
          >
            <Icon name="ClipboardList" size={20} />
            <span className="font-medium">Инвентаризация</span>
          </Button>
        </div>
      </main>
    </div>
