...
const ScanBarcode = () => {
  ...
  return (
    <div className="fixed inset-0 bg-black flex flex-col">
      ...
      {status === "unsupported" ? (
        ...
        <div className="flex-1 relative overflow-hidden">
          ...
          {status === "scanning" && !error && (
            <button
              className="absolute bottom-24 left-1/2 -translate-x-1/2 w-16 h-16 rounded-full bg-white flex items-center justify-center shadow-lg active:scale-95 transition-transform"
              onClick={takePhoto}
      ...
      <div className="bg-black/90 px-4 py-3 max-h-40 overflow-y-auto">
        {lastScanned && (
          <p className="text-green-400 text-sm font-medium flex items-center justify-center gap-2 mb-2">
            <Icon name="Check" size={16} />
            {lastScanned}
          </p>
        )}
        {collected.length > 0 ? (
          <div className="flex flex-wrap gap-1.5">
            {collected.map((bc, i) => (
              <span key={i} className="inline-flex items-center gap-1 px-2 py-1 rounded-lg bg-white/10 text-xs text-white">
                {bc}
                <button onClick={() => removeBarcode(i)}>
                  <Icon name="X" size={12} className="text-white/60" />
                </button>
              </span>
            ))}
          </div>
